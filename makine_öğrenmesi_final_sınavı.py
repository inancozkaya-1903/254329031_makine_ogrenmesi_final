...
print("\nSonsuz değer kontrolü:")
# Önce sonsuz değerleri NaN'a çevirmeden doğrudan kontrol ediyoruz.
inf_count = np.isinf(df[feature_cols].to_numpy(dtype=float)).sum()
...
if missing_count == 0 and inf_count == 0:
    print("\nSonuç: Eksik veya sonsuz değer görünmüyor.")
else:
    print("\nUyarı: Eksik veya sonsuz değer mevcut. Ön işleme pipeline'ında median imputation ve inf-to-NaN dönüşümü uygulanacaktır.")
...
# Final independent testte en iyi çıkan sample-level modeli temel alıyoruz.
# Eğer en iyi model Soft Voting Ensemble ise, ensemble'ın tekil MRMR seti olmadığı için RBF-SVM fallback olarak kullanılır.
if "best_final_model_name" in globals() and best_final_model_name in final_optuna_studies:
    BEST_MODEL_FOR_STABILITY = best_final_model_name
else:
    BEST_MODEL_FOR_STABILITY = "RBF-SVM"

STABILITY_MRMR_FEATURE_COUNT = final_optuna_studies[
    BEST_MODEL_FOR_STABILITY
].best_params["n_mrmr_features"]
...
for split_id in range(N_STABILITY_SPLITS):
    ...
    # Ön işleme yalnızca stability train fold'una fit edilir.
    ...
    # MRMR yalnızca stability train fold'u üzerinde yapılır.
    ...
    for feature in selected_features_stab:
        stability_records.append({
            "split_id": split_id + 1,
            "feature": feature,
            "base_model": BEST_MODEL_FOR_STABILITY
        })
...
feature_stability_df["base_model"] = BEST_MODEL_FOR_STABILITY
...
safe_stability_model_name = BEST_MODEL_FOR_STABILITY.lower().replace(" ", "_").replace("-", "_")

feature_stability_path = os.path.join(
    OUTPUT_DIR,
    f"mrmr_feature_stability_analysis_{safe_stability_model_name}.csv"
)
feature_stability_df.to_csv(feature_stability_path, index=False)

# Raporla geriye uyumluluk için eski genel isimle de kaydediyoruz.
feature_stability_legacy_path = os.path.join(
    OUTPUT_DIR,
    "mrmr_feature_stability_analysis.csv"
)
feature_stability_df.to_csv(feature_stability_legacy_path, index=False)

print("\nFeature stability tablosu kaydedildi:")
print(feature_stability_path)
print(feature_stability_legacy_path)
...
# =========================
# HÜCRE 55: Klinik Recall Odaklı Threshold Seçimi - Final Best Model
# =========================

# Klinik threshold analizini final sample-level en iyi model üzerinde yapıyoruz.
# Eğer final en iyi model ensemble olursa, ensemble validation olasılığı kullanılır.
if "best_final_model_name" in globals():
    if best_final_model_name in validation_prediction_store:
        CLINICAL_MODEL_NAME = best_final_model_name
        clinical_val_proba = validation_prediction_store[CLINICAL_MODEL_NAME]["val_calibrated_proba"]
        clinical_standard_threshold = model_thresholds[CLINICAL_MODEL_NAME]

    elif best_final_model_name == "Soft Voting Ensemble":
        CLINICAL_MODEL_NAME = "Soft Voting Ensemble"
        clinical_val_proba = ensemble_val_proba
        clinical_standard_threshold = ensemble_threshold
...
print("Klinik threshold analizi yapılan model:", CLINICAL_MODEL_NAME)
print("Standart Macro-F1 threshold:", clinical_standard_threshold)
print("Klinik recall odaklı threshold:", clinical_recall_threshold)

print("\nValidation üzerinde seçilen klinik threshold satırı:")
...
safe_clinical_model_name = CLINICAL_MODEL_NAME.lower().replace(" ", "_").replace("-", "_").replace("+", "_")

clinical_threshold_path = os.path.join(
    OUTPUT_DIR,
    f"clinical_recall_threshold_sweep_{safe_clinical_model_name}_validation.csv"
)
clinical_threshold_df.to_csv(clinical_threshold_path, index=False)

print("\nKlinik recall threshold sweep tablosu kaydedildi:")
...
# =========================
# HÜCRE 56: Klinik Threshold Test Değerlendirmesi - Final Best Model
# =========================

# Klinik analiz yapılan modelin test olasılıkları alınır.
clinical_test_proba = test_prediction_store[CLINICAL_MODEL_NAME]["calibrated_proba"]

standard_threshold = clinical_standard_threshold
clinical_threshold = clinical_recall_threshold

# Standart threshold sonucu
clinical_test_pred_standard = (clinical_test_proba >= standard_threshold).astype(int)

standard_metrics = evaluate_final_binary_classification(
    y_true=y_test,
    y_pred=clinical_test_pred_standard,
    y_proba=clinical_test_proba
)

standard_cm = confusion_matrix(y_test, clinical_test_pred_standard)

# Klinik recall threshold sonucu
clinical_test_pred_recall = (clinical_test_proba >= clinical_threshold).astype(int)

clinical_metrics = evaluate_final_binary_classification(
    y_true=y_test,
    y_pred=clinical_test_pred_recall,
    y_proba=clinical_test_proba
)

clinical_cm = confusion_matrix(y_test, clinical_test_pred_recall)

clinical_comparison_df = pd.DataFrame([
    {
        "Model": CLINICAL_MODEL_NAME,
...
print(f"{CLINICAL_MODEL_NAME} standart threshold vs klinik recall threshold test karşılaştırması:")
...
clinical_comparison_path = os.path.join(
    OUTPUT_DIR,
    f"{safe_clinical_model_name}_standard_vs_clinical_threshold_test_comparison.csv"
)
clinical_comparison_df.to_csv(clinical_comparison_path, index=False)

# Raporla geriye uyumluluk için eski genel isimle de kaydediyoruz.
clinical_comparison_legacy_path = os.path.join(
    OUTPUT_DIR,
    "standard_vs_clinical_threshold_test_comparison.csv"
)
clinical_comparison_df.to_csv(clinical_comparison_legacy_path, index=False)

print("\nKarşılaştırma tablosu kaydedildi:")
print(clinical_comparison_path)
print(clinical_comparison_legacy_path)
...
for setting_name, cm_data, threshold_value in threshold_settings:
    plt.figure(figsize=(5, 4))
    plt.imshow(cm_data)
    plt.title(f"{CLINICAL_MODEL_NAME}\n{setting_name}\nThreshold={threshold_value:.2f}")
    plt.xlabel("Predicted Label")
    plt.ylabel("True Label")
    plt.xticks([0, 1], ["Normal", "Papilodem"])
    plt.yticks([0, 1], ["Normal", "Papilodem"])
    ...
    safe_setting_name = setting_name.lower().replace(" ", "_").replace("-", "_")

    fig_path = os.path.join(
        FIGURE_DIR,
        f"{safe_clinical_model_name}_{safe_setting_name}_confusion_matrix.png"
    )
...
Bu bölümde destekleyici/bonus açıklanabilirlik analizi kapsamında RBF-SVM modeli için SHAP analizi hazırlanacaktır.

Final sample-level model seçimi bağımsız test sonuçlarına göre yapılmaktadır. Bu nedenle SHAP analizi, final model kararının yerine geçen bir analiz değildir. Buradaki amaç, güçlü performans gösteren kernel tabanlı RBF-SVM modelinin tahminlerinde hangi radyomik özelliklerin daha etkili olduğunu incelemektir.

RBF-SVM doğrusal olmayan karar sınırları oluşturabildiği için doğrudan katsayı temelli yorumlanması mümkün değildir. Bu nedenle modelden bağımsız çalışan Kernel SHAP yaklaşımı kullanılacaktır. SHAP analizi, modelin pozitif sınıf olan Papilödem olasılığını üretirken hangi özelliklerden daha fazla etkilendiğini global düzeyde yorumlamaya yardımcı olur.

Hesaplama maliyetini kontrol altında tutmak için sınırlı sayıda background örneği ve test örneği seçilecektir. Bu analiz raporda açıklanabilir yapay zeka kapsamında destekleyici bulgu olarak değerlendirilecektir.
...
Bu bölümde destekleyici/bonus açıklanabilirlik analizi kapsamında RBF-SVM modeli için LIME analizi uygulanacaktır.

LIME, global model başarısından farklı olarak tekil bir örnek özelinde model kararını açıklamaya odaklanır. Bu nedenle SHAP analiziyle birlikte kullanıldığında modelin hem genel özellik önem sıralamasını hem de bireysel tahmin davranışını yorumlamaya yardımcı olur.

Bu çalışmada bir normal ve bir papilödem test örneği seçilerek, RBF-SVM modelinin ilgili örnekler için Papilödem sınıfı olasılığını hangi özellik kurallarıyla desteklediği veya zayıflattığı incelenecektir.

Bu analiz final model seçimini değiştirmek için değil, model tahminlerinin daha anlaşılır ve savunulabilir hale getirilmesi için eklenmiştir. LIME çıktıları klinik nedensellik kanıtı olarak değil, modelin karar mekanizmasını açıklamaya yönelik yerel açıklanabilirlik çıktısı olarak yorumlanacaktır.
...
Bu bölümde destekleyici/bonus analiz olarak RBF-SVM modeli için nested cross-validation uygulanacaktır.

Nested cross-validation, model performansının tek bir train-validation-test bölmesine fazla bağımlı olup olmadığını değerlendirmek için kullanılan daha katı bir doğrulama yaklaşımıdır. Bu analizde amaç, final independent test sonucunu değiştirmek değil, modelin farklı hasta bazlı bölmelerde ne kadar kararlı performans verdiğini incelemektir.

Nested cross-validation iki seviyeli bir yapıdan oluşur:

* Outer loop: Modelin farklı hasta bazlı test foldları üzerindeki performansını ölçer.
* Inner loop: Her outer training fold içinde hiperparametre seçimi yapar.

Bu çalışmada hesaplama maliyetini kontrol altında tutmak için nested CV yalnızca RBF-SVM modeli üzerinde uygulanmıştır. RBF-SVM, doğrusal olmayan karar sınırları kurabilmesi ve final model karşılaştırmalarında güçlü performans göstermesi nedeniyle bu bonus genellenebilirlik analizi için seçilmiştir.

Nested CV sonuçları, bağımsız test performansına ek olarak yorumlanacaktır. Eğer nested CV performansı final test sonucundan daha düşük çıkarsa, bu durum çalışmanın hatalı olduğu anlamına gelmez; sınırlı hasta/grup sayısında performansın hasta bölmelerine duyarlı olabileceğini gösterir. Bu nedenle nested CV bulguları, modelin daha geniş ve dış veri setleriyle doğrulanması gerektiğini değerlendirmek için kullanılacaktır.
...
# =========================
# HÜCRE 73: MRMR Özellik Seçiminin Performansa Etkisinin Değerlendirilmesi
# =========================

"""
Bu hücrede MRMR özellik seçiminin model performansına etkisi değerlendirilmektedir.

Önceki versiyonda karşılaştırma yalnızca RBF-SVM üzerinden yapılıyordu.
Bu güncel versiyonda karşılaştırma öncelikle final independent testte en iyi çıkan
sample-level model üzerinden yapılır. Eğer final en iyi model LR ise MRMR etkisi LR için,
RBF-SVM ise RBF-SVM için hesaplanır.

Ek olarak, RBF-SVM güçlü ve destekleyici bir model olduğu için final best modelden farklıysa
RBF-SVM karşılaştırması da bonus olarak korunur.

Karşılaştırma mantığı:
1. Aynı model hiperparametreleri kullanılır.
2. Bir senaryoda preprocessing sonrası kalan tüm özellikler kullanılır.
3. Diğer senaryoda MRMR ile seçilen özellikler kullanılır.
4. Threshold validation set üzerinde seçilir.
5. Independent test seti yalnızca final karşılaştırma için kullanılır.
"""
...
def get_primary_mrmr_comparison_models():
    """
    MRMR karşılaştırması yapılacak modelleri belirler.
    Ana model: final independent testte en iyi çıkan sample-level model.
    Ek model: Ana model RBF-SVM değilse RBF-SVM destekleyici olarak eklenir.
    """

    comparison_models = []

    if "best_final_model_name" in globals() and best_final_model_name in final_optuna_studies:
        comparison_models.append(best_final_model_name)
    else:
        # Güvenli fallback
        if "LR" in final_optuna_studies:
            comparison_models.append("LR")
        else:
            comparison_models.append("RBF-SVM")

    # RBF-SVM bonus karşılaştırması korunur.
    if "RBF-SVM" in final_optuna_studies and "RBF-SVM" not in comparison_models:
        comparison_models.append("RBF-SVM")

    return comparison_models


def evaluate_model_mrmr_effect(model_name, use_mrmr):
    """
    Belirli bir model için MRMR kullanımı / kullanılmaması durumunu karşılaştırır.

    use_mrmr=False:
        Preprocessing sonrası kalan tüm özellikler kullanılır.

    use_mrmr=True:
        Optuna'nın seçtiği n_mrmr_features sayısı kadar MRMR özelliği kullanılır.
    """

    best_params = final_optuna_studies[model_name].best_params.copy()

    preprocessor_local = RadiomicsPreprocessor(
        variance_threshold=1e-8,
        corr_threshold=0.95
    )

    X_train_prep_local = preprocessor_local.fit_transform(train_df[feature_cols])
    X_val_prep_local = preprocessor_local.transform(val_df[feature_cols])
    X_test_prep_local = preprocessor_local.transform(test_df[feature_cols])
    ...
    if use_mrmr:
        ...
        selected_features_local, mrmr_table_local = mrmr_feature_selection(
            X=X_train_prep_local,
            y=y_train_local,
            n_features=n_mrmr_features,
            redundancy_weight=0.5,
        ...
        setting_name = "With MRMR"

    else:
        selected_features_local = list(X_train_prep_local.columns)
        mrmr_table_local = None
        setting_name = "Without MRMR"

    X_train_selected_local = X_train_prep_local[selected_features_local]
    X_val_selected_local = X_val_prep_local[selected_features_local]
    X_test_selected_local = X_test_prep_local[selected_features_local]

    model_local = build_model_from_best_params(
        model_name=model_name,
        best_params=best_params
    )

    model_local.fit(X_train_selected_local, y_train_local)

    val_proba_local = model_local.predict_proba(X_val_selected_local)[:, 1]
    test_proba_local = model_local.predict_proba(X_test_selected_local)[:, 1]

    best_threshold_local, threshold_df_local = optimize_threshold_on_validation(
        y_true=y_val_local,
        y_proba=val_proba_local,
        step=0.01
    )

    test_pred_local = (test_proba_local >= best_threshold_local).astype(int)

    metrics_local = evaluate_final_binary_classification(
        y_true=y_test_local,
        y_pred=test_pred_local,
        y_proba=test_proba_local
    )

    cm_local = confusion_matrix(y_test_local, test_pred_local)

    result_record = {
        "Model": model_name,
        "Setting": setting_name,
        "Feature Count": len(selected_features_local),
    ...
    return result_record, threshold_df_local, mrmr_table_local


# ---------------------------------------------------------
# MRMR karşılaştırmasının çalıştırılması
# ---------------------------------------------------------

mrmr_comparison_models = get_primary_mrmr_comparison_models()

print("MRMR etkisi karşılaştırılacak modeller:")
print(mrmr_comparison_models)

all_mrmr_effect_records = []

for model_name in mrmr_comparison_models:
    print("\n" + "=" * 80)
    print(f"MRMR etkisi değerlendiriliyor: {model_name}")
    print("=" * 80)

    model_records = []

    for use_mrmr in [False, True]:
        result_record, threshold_df_temp, mrmr_table_temp = evaluate_model_mrmr_effect(
            model_name=model_name,
            use_mrmr=use_mrmr
        )

        model_records.append(result_record)
        all_mrmr_effect_records.append(result_record)

        setting_safe = result_record["Setting"].lower().replace(" ", "_")
        model_safe = model_name.lower().replace(" ", "_").replace("-", "_").replace("+", "_")

        threshold_path = os.path.join(
            OUTPUT_DIR,
            f"mrmr_effect_threshold_search_{model_safe}_{setting_safe}.csv"
        )
        threshold_df_temp.to_csv(threshold_path, index=False)

        if mrmr_table_temp is not None:
            mrmr_table_path = os.path.join(
                OUTPUT_DIR,
                f"mrmr_effect_selected_features_{model_safe}.csv"
            )
            mrmr_table_temp.to_csv(mrmr_table_path, index=False)

    model_mrmr_effect_df = pd.DataFrame(model_records)
    ...
    model_mrmr_effect_df = model_mrmr_effect_df[ordered_cols]

    print(f"\n{model_name} için MRMR vs No-MRMR karşılaştırması:")
    display(model_mrmr_effect_df)

    model_safe = model_name.lower().replace(" ", "_").replace("-", "_").replace("+", "_")

    model_mrmr_effect_path = os.path.join(
        OUTPUT_DIR,
        f"mrmr_vs_no_mrmr_{model_safe}_test_comparison.csv"
    )
    model_mrmr_effect_df.to_csv(model_mrmr_effect_path, index=False)

    print("Model bazlı MRMR karşılaştırma tablosu kaydedildi:")
    print(model_mrmr_effect_path)

    # Basit karşılaştırma grafiği
    plt.figure(figsize=(7, 4))
    plt.bar(
        model_mrmr_effect_df["Setting"],
        model_mrmr_effect_df["Macro-F1"]
    )
    plt.title(f"MRMR Etkisi - {model_name} - Independent Test Macro-F1")
    plt.xlabel("Setting")
    plt.ylabel("Macro-F1")
    plt.ylim(0, 1.05)
    plt.tight_layout()

    mrmr_effect_fig_path = os.path.join(
        FIGURE_DIR,
        f"mrmr_effect_macro_f1_{model_safe}.png"
    )
    plt.savefig(mrmr_effect_fig_path, dpi=300, bbox_inches="tight")
    plt.show()

    print("MRMR etki grafiği kaydedildi:")
    print(mrmr_effect_fig_path)


# ---------------------------------------------------------
# Tüm karşılaştırmaları tek tabloda kaydetme
# ---------------------------------------------------------

mrmr_effect_df = pd.DataFrame(all_mrmr_effect_records)

mrmr_effect_df = mrmr_effect_df[
    [
        "Model",
        "Setting",
        "Feature Count",
...
print("\nTüm modeller için MRMR etkisi karşılaştırması:")
display(mrmr_effect_df)

mrmr_effect_all_path = os.path.join(
    OUTPUT_DIR,
    "mrmr_vs_no_mrmr_final_best_and_rbf_svm_test_comparison.csv"
)
mrmr_effect_df.to_csv(mrmr_effect_all_path, index=False)

# Raporla geriye uyumluluk için eski genel isimle de kaydediyoruz.
mrmr_effect_legacy_path = os.path.join(
    OUTPUT_DIR,
    "mrmr_vs_no_mrmr_test_comparison.csv"
)
mrmr_effect_df.to_csv(mrmr_effect_legacy_path, index=False)

print("\nTüm MRMR karşılaştırma tablosu kaydedildi:")
print(mrmr_effect_all_path)
print(mrmr_effect_legacy_path)
